...
class InstructorRepository {
  ...
  Future<Response?> getInstructorProfile(int id) async {
    return await apiClient.getData(AppConstants.instructorProfile);
  }

  Future<Response?> getCourseListOfInstructors(
      {required int id, required int page}) async {
    return await apiClient
        .postData("${AppConstants.instructorCourseList}?page=$page", {});
  }

  Future<Response?> getPendingCourseListOfInstructors({required int id, required int page}) async{
    return await apiClient
        .getData("${AppConstants.instructorPendingCourseList}?page=$page");
  }

  Future<Response?> getStudentsList(
      {required int id, required int page}) async {
    return await apiClient
        .getData("${AppConstants.instructorStudentList}?page=$page");
  }
